...
Definir uma estratégia de testes rastreável para comprovar regras de domínio, casos de uso, persistência, integrações Google e fluxos críticos do frontend.
...
| RNF04 | simular indisponibilidade Drive/Gmail/Calendar e verificar operação local |
...
| Notificações | prévia, confirmação, erro e status de envio |
...
2. Product Owner cria item, ordena backlog e seleciona item para Sprint.
3. Membro move item no quadro respeitando WIP.
4. Membro registra bloqueio e o resolve.
5. Membro envia arquivo para Drive de teste.
6. Coordenador envia notificação Gmail de teste.
7. Coordenador cria evento interno e sincroniza Calendar de teste.
8. Serviço Google falha e o sistema mantém os dados locais.

## 11. Dados e ambientes

- **Unitário:** objetos em memória e dados sintéticos.
- **Aplicação:** repositories e gateways fake.
- **Integração:** banco PostgreSQL de teste e contas Google de desenvolvimento.
...
| RF17-RF20 | UC10-UC11 | A10-A11, contrato Calendar, E2E07 |
| RNF01-RNF03 | UC01, UC08, UC09, UC12 | segurança e autorização |
| RNF04-RNF05 | UC08, UC09, UC11 | falhas, retry e idempotência |
